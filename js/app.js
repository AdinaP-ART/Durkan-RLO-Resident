...
/* ============================================================
   RESIDENT NAV
============================================================ */
const resPageMap = { 1:'rp-home', 2:'rp-appts', 4:'rp-message', 6:'rp-feedback', 7:'rp-during', 8:'rp-colours', 9:'rp-updates', 10:'rp-infopack', 11:'rp-maintenance', 12:'rp-my-letters' };
const resNavDef  = [
  ...
  { i:8,  icon:'ti-photo',          label:'Examples of Completed Work' },
  { i:9,  icon:'ti-speakerphone',   label:'Updates & Events' },
  { i:12, icon:'ti-mail-opened',    label:'My Letters' },
  { i:4,  icon:'ti-mail',           label:'Message & Report an Issue' },
...
/* ============================================================
   UTILITY
============================================================ */
function genCode(flatOrUprn) {
  // Prefer a clean alphanumeric base from a UPRN if one looks present (letters+digits, 8+ chars);
  // otherwise fall back to pulling digits out of a flat/address name.
  const raw = (flatOrUprn || '').trim();
  const looksLikeUprn = /^[A-Za-z0-9]{6,}$/.test(raw.replace(/\s/g,''));
  const base = looksLikeUprn ? raw.replace(/\s/g,'').slice(-6).toUpperCase() : raw.replace(/\D/g,'').padStart(2,'0');
...
function parseRows(rows, filename) {
  // UPRN is the true unique reference for a property — match on that first.
  // If a row has no UPRN (e.g. quick manual entries), fall back to matching
  // on Address No + Address together, so different roads don't collide.
  const norm = s => (s || '').replace(/\s+/g, '').toLowerCase();
  const parsed = rows.map(r => {
    const uprn = getCol(r,'UPRN','Uprn','URPN','PropertyRef','Reference');
    ...
    const existing = uprn
      ? db.schedule.find(e => e.uprn && norm(e.uprn) === norm(uprn))
  ...
  parsed.forEach(e => { if (!e.accessCode) e.accessCode = genCode(e.uprn || e.flat); });
  db.schedule = parsed;
  renderEntryTable();
...
function renderEntryTable() {
  ...
  tbody.innerHTML = db.schedule.map((e,i) => `
    <tr>
      <td><input style="width:90px;border:1px solid var(--dg);border-radius:5px;padding:3px 6px;font-size:11px" value="${e.uprn||''}" placeholder="UPRN" onchange="db.schedule[${i}].uprn=this.value"/></td>
      ...
      <td><select style="width:170px;border:1px solid var(--dg);border-radius:5px;padding:3px 6px;font-size:11px" onchange="db.schedule[${i}].workType=this.value">${WORK_TYPE_OPTIONS.map(w=>`<option${e.workType===w?' selected':''}>${w}</option>`).join('')}${WORK_TYPE_OPTIONS.includes(e.workType)?'':`<option selected>${e.workType||'—'}</option>`}</select></td>
...
function addEntry() {
  db.schedule.push({uprn:'',flat:'Address',address:'',resident:'',workType:'Kitchen only',accessCode:genCode('Address'),slots:[],status:'pending',confirmedDate:'',locked:false,contactLog:[]});
  renderEntryTable();
}
...
async function sendTomorrowDuringReminders() {
  ...
  for (const e of matches) {
    ...
    const result = await sendSMS(mobile, `Hi ${e.resident.split(' ')[0]}, tomorrow the ${e.trade} is scheduled at ${e.flat} in the ${e.timeframe}. Please ensure access is available. Durkan Regen. This is an automated message — please do not reply to this number.`);
...
function markLetterViewed(letterId) {
  // Only sets viewed_at the first time — .is('viewed_at', null) means a
  // letter's original view time is preserved on any later re-opens.
  sb.from('letters_sent').update({ viewed_at: new Date().toISOString() }).eq('id', letterId).is('viewed_at', null)
    .then(({ error }) => { if (error) console.warn('Mark letter viewed failed:', error.message); });
}
...
async function renderMyLetters() {
  ...
  try {
    ...
    body.innerHTML = data.map(l => `
      <div class="faq-item" onclick="toggleFaq(this);markLetterViewed('${l.id}')">
    ...
    if (openLetterAfterLoad) {
      ...
      if (idx > -1) {
        ...
        if (items[idx]) { toggleFaq(items[idx]); markLetterViewed(data[idx].id); }
      }
    }
  } catch (err) {
      ...
      <div style="background:#f2f3f5;border-radius:9px;padding:10px 12px;margin-bottom:8px">
        <div style="display:flex;justify-content:space-between;align-items:center;cursor:pointer" onclick="const c=document.getElementById('fl-body-${i}-${li}');c.style.display=c.style.display==='none'?'block':'none'">
          <div>
            ...
            <span style="font-size:10px;color:#6b6b6b">${new Date(l.created_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</span><br>
            <span style="font-size:10px;color:${l.viewed_at?'#3b6d11':'#854f0b'};font-weight:600">${l.viewed_at ? '✓ Viewed '+new Date(l.viewed_at).toLocaleDateString('en-GB',{day:'numeric',month:'short'}) : 'Not yet viewed'}</span>
          </div>
    ...
    list.innerHTML = data.length ? data.map(l => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--dg);font-size:12px">
        <div>
          ...
          <span style="color:var(--dgd);font-size:11px">${l.title}${l.stage?' ('+l.stage+' Request)':''}</span><br>
          <span style="font-size:10px;color:${l.viewed_at?'#3b6d11':'#854f0b'};font-weight:600">${l.viewed_at ? '✓ Viewed '+new Date(l.viewed_at).toLocaleDateString('en-GB',{day:'numeric',month:'short'}) : 'Not yet viewed'}</span>
        </div>
